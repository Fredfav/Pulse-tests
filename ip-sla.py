...
def select_iperf_and_run(server,port,protocol,ToS,bandwidth,duration):
    status = False
    outbytes = ['']
    iperf_options = ' -c ' + server + ' -p ' + port + ' -t ' + str(duration) + ' -S ' + ToS
    if protocol == 'udp':
        iperf_options += ' -u'
...
def driver(server, port, protocol, ToS, bandwidth, test_duration):
    ...
    status, iperf_output = select_iperf_and_run(server,port,protocol,ToS,bandwidth,test_duration)
    if not status:
        return result

    status, sender, receiver = get_stats(iperf_output)
    if status:
...
def run(testConfig):
    ...
    return json.dumps(driver(config['server'], config['port'], 'udp', config['ToS'], config['bandwidth'], config['test_duration']))