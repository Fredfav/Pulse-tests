<#
	...
	.PARAMETER SleepBeforeEndingCitrix
		The time in seconds to sleep before ending the Citrix session. Default is 5.
    ...
    .NOTES
		The following configuration in Internet Explorer has to be performed.
		Step 1:
			In Internet Explorer click on settings.
			Click on View Downloads or press Control+J
			In the View Downloads window, click Options in the lower left hand corner.
			In the Options window, uncheck Notify me when my downloads are complete.

		Step 2:
			In Internet Explorer, Go to settings and click Internet Options.
			Click the Security tab.
			Under Security level, add the StoreFront web site URL in the Intranet zone.
...
Param (
	...
	[Parameter(Mandatory=$false)] [int]$SleepBeforeEndingCitrix = 5,
...
function Write-SFLauncherHeader {
    $infoMessage = @"
                           ...
                           *****************************************************
                           *****    Citrix Solutions SF Launcher Script    *****
                           *****        -Console output:     {0,-6}        *****
                           *****        -Log output:         {1,-6}        *****
                           *****************************************************
...
function Write-ToSFLauncherLog {
    ...
    Process {
        $Message = [DateTime]::Now.ToString("MM/dd/yyy HH:mm:ss.fff # ") + $Message +"`n"
...
function Wait-ForPageReady {
    while ($internetExplorer.ReadyState -ne 4) {
		# debug to print out the state of IE
        Write-ToSFLauncherLog "Internet Explorer: BUSY + $internetExplorer.ReadyState"
        Start-Sleep 1
    }
}
...
function Terminate-Citrix {
	Write-ToSFLauncherLog "Sleeping $SleepBeforeEndingCitrix seconds before terminating Citrix session..."
	Start-Sleep -Seconds $SleepBeforeEndingCitrix
	
	Write-ToSFLauncherLog "Ending Citrix process..."
	$wficaList = @()
    Get-Process wfica32 -ErrorAction SilentlyContinue | select id | % { $wficaList += $_.id }
	if ($wficaList -ne $null) {
		foreach ($id in $wficaList) {
			"Killing wfica32.exe with PID $id" | Write-ToSFLauncherLog
			Stop-Process -Id $id 
		}
	} else {
		Write-ToSFLauncherLog "WARNING: No Citrix session found"
	}
}
